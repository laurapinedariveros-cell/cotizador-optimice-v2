...
import { emptyRevisoria, emptyPT, emptyOutsourcing, emptyActivosFijos, emptyExistencias, emptyAuditoria, sectionCompleteness } from "../lib/model";
...
const SERVICIO_LABEL = {
  ...
  auditoria: "AUDITORÍA EXTERNA / DUE DILIGENCE",
};
...
export default function ClientPage() {
  ...
  const dataByTipo = { revisoria, pt, outsourcing, activosFijos, existencias, auditoria };
  const setterByTipo = {
  ...
  const startNew = (t) => {
    ...
    else setAuditoria(emptyAuditoria());
    setView("form");
  };

  const FOLIO_PREFIX = { revisoria: "RF", pt: "PT", outsourcing: "OC", activosFijos: "AF", existencias: "EX", auditoria: "AU" };
        ...
        <div className="op-landing">
          ...
          <div className="op-choice-grid">
            ...
            <button className="op-choice" onClick={() => startNew("auditoria")}>
              ...
              <h3>Auditoría Externa / Due Diligence</h3>
              <p>Auditoría de estados financieros, de un rubro específico, o due diligence financiero.</p>
            </button>
          </div>
        </div>
      )}
