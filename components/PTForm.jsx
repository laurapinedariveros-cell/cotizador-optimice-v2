...
import { Field, YesNo, CheckChip, Card } from "./Brand";
    ...
    <>
      <Card title="Información general" eyebrow="Sección 1" pct={sectionCompleteness(data.general)}>
        <div className="op-grid2">
          ...
          <Field label="Ingresos del periodo"><input type="number" className="op-input" value={data.general.ingresosPeriodo} onChange={(e) => set("general", { ingresosPeriodo: e.target.value })} /></Field>
          <Field label="Utilidad del periodo"><input type="number" className="op-input" value={data.general.utilidadPeriodo} onChange={(e) => set("general", { utilidadPeriodo: e.target.value })} /></Field>
          <Field label="Valor de patrimonio"><input type="number" className="op-input" value={data.general.valorPatrimonio} onChange={(e) => set("general", { valorPatrimonio: e.target.value })} /></Field>
        </div>
      </Card>
      ...
      <Card title="Consideraciones generales" eyebrow="Sección 2" pct={sectionCompleteness(data.consideraciones)}>
        ...
        <Field label="¿Qué le gustaría cotizar? (puede elegir varias opciones)">
          <div className="op-chip-row" style={{ flexWrap: "wrap" }}>
            <CheckChip label="Informe Local" value={data.consideraciones.cotizarInformeLocal} onChange={(v) => set("consideraciones", { cotizarInformeLocal: v })} />
            <CheckChip label="Informe Maestro" value={data.consideraciones.cotizarInformeMaestro} onChange={(v) => set("consideraciones", { cotizarInformeMaestro: v })} />
            <CheckChip label="Informe País por País" value={data.consideraciones.cotizarInformePaisPorPais} onChange={(v) => set("consideraciones", { cotizarInformePaisPorPais: v })} />
            <CheckChip label="Declaración informativa de precios de transferencia" value={data.consideraciones.cotizarDeclaracionInformativa} onChange={(v) => set("consideraciones", { cotizarDeclaracionInformativa: v })} />
          </div>
        </Field>
      </Card>
